...
static int mpro_conn_init(struct mpro_device *mpro)
{
	...
	conn_name = kasprintf(GFP_KERNEL, "USB-Display-%d-%s", udev->bus->busnum, udev->devpath);
	if (!conn_name)
		return -ENOMEM;

	pr_info("mpro: assigning connector name: %s\n", conn_name);

	kfree(mpro->conn.name);  // Free old autogenerated name
	mpro->conn.name = conn_name;
